...
namespace nsoftware.GdriveDemo
{
	partial class GdriveViewController : UIViewController
	{
		...
		async partial void UIButton30_TouchUpInside(UIButton sender)
		{
			var oauth = new Oauth(this);
			...
			oauth.OnLaunchBrowser += (senders, es) =>
			{
				InvokeOnMainThread(() =>
				{
					...
					vc.Add(webView);
					webView.LoadRequest(new NSUrlRequest(new NSUrl(es.URL)));
			...
			try
			{
				...
				var url = await oauth.GetAuthorizationAsync();
